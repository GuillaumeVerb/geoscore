...
- **Scoring is frozen at `scoring-v1-cal03` / `ruleset-v1-cal03`.** Do not change the deterministic engine for broad re-calibration; only targeted fixes when a case is clearly wrong. **Partial scans** with strong limitations and almost no issues/recos get **product-layer fallback** issues/recommendations (post-scoring, same API shape) — do not duplicate that logic inside `score_minimal.py`.
